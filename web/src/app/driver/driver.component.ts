...
export class DriverComponent implements OnInit, OnDestroy {
  ...
  updateStatus(status: DeliveryStatus) {
    this.updating = true;
    this.apiService
      .updateDelivery(this.delivery?.delivery_id!, {
        status: status,
      })
      .pipe(
        catchError((e) => {
  ...
  pullPosition() {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        this.pollPositionTimeout = setTimeout(
          () => this.pullPosition(),
          20 * 1000
        );
      },
      () => {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to get your position, please check your permissions',
        });
      },
  ...
  ngOnDestroy(): void {
    clearTimeout(this.pollPositionTimeout);
  }

  search() {
    this.searching = true;
    this.apiService
      .findDelivery(this.searchForm.value.delivery_id!)
      .pipe(
        catchError((e) => {
          let message = 'Something went wrong';
          console.error(e);
  ...
  get markerPositions() {
    ...
    return [
      {
        position: this.delivery.package.from_location,
        title: 'Pickup location',
        options: {},
        content: this.pickupPinElement,
      },
      {
        position: this.delivery.package.to_location,
        title: 'Destination',
        options: {},
